...
export class FinancialService implements IFinancialService {
  constructor(
    ...
    private patientFinancialRepository: any,
  ...
  async getMonthlyConsolidated(userId: string, month: number, year: number): Promise<FinancialConsolidated> {
    ...
    const cashFlow: FinancialConsolidated['cashFlow'] = [];
    const patientIds = new Set<string>();

    // Processa registros da clínica
    clinicFinancials.forEach((item: Financial) => {
    ...
    patientFinancials.forEach((item: PatientFinancial) => {
      const amount = item.amount;
      const method = item.paymentMethod || 'other';
    ...
    cashFlow.forEach(flow => {
      ...
      if (flow.source === 'patient') {
        const original = patientFinancials.find((p: PatientFinancial) => p.id === flow.id);
